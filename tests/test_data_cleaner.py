...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        self.assertEqual(result[["name", "age"]].isna().sum().sum(), 0,"El DataFrame resultante no debería tener valores faltantes en las columnas especificadas")
        self.assertLess(len(result), len(df),"El DataFrame resultante debería tener menos filas que el original")
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        pdt.assert_series_equal(result["city"], df2["city"],"Las columnas no especificadas deberían permanecer sin cambios")
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        self.assertNotIn(120, result["age"].values, "El valor extremo debería ser eliminado del resultado")
        self.assertIn(25, result["age"].values, "Los valores normales deberían permanecer en el resultado")
