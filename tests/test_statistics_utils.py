...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        npt.assert_allclose(result, expected, rtol=1e-7, atol=1e-7)
        self.assertEqual(result.shape, expected.shape, "El resultado debería tener la forma esperada")
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        utils = StatisticsUtils()
        arr = [10, 20, 30, 40]
        resultado = utils.zscore(arr)
        self.assertAlmostEqual(resultado.mean(), 0.0, places=10,msg="La media del resultado debería ser aproximadamente 0")
        self.assertAlmostEqual(resultado.std(), 1.0, places=10,msg="La desviación estándar del resultado debería ser aproximadamente 1")
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        self.assertAlmostEqual(result.min(), 0.0, places=10, msg="El valor mínimo del resultado debería ser 0.0")
        self.assertAlmostEqual(result.max(), 1.0, places=10, msg="El valor máximo del resultado debería ser 1.0")
        npt.assert_allclose(result, expected, rtol=1e-10, atol=1e-10)
